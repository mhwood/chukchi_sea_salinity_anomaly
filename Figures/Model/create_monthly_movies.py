...
def make_movie_from_commandline(config_dir, results_dir, var_name, year):
    pwd = os.getcwd()
    ...
    output_name = var_name + '_' + str(year)+'_'+experiment + '.mp4'
...
for results_dir in ['results_daily_hydrology', 'results_control']:
    ...
    for var_name in ['Theta','Salt','SIheff','SIarea']:
        ...
        for year in [2023, 2024]:
            experiment = results_dir.replace('results_', '')
            movie_file = var_name + '_' + str(year)+'_'+experiment + '.mp4'
            if movie_file not in os.listdir(os.path.join(config_dir, results_dir.replace('results','plots'), 'Movies')):
                ...
                make_movie_from_commandline(config_dir, results_dir, var_name, year)