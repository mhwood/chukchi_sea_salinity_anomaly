...
def plot_monthly_experiment_anomaly_DBO_fields(project_folder, experiment, year, transect, var_name, var_grid_control,
                                               dec_yrs, Z, var_grid,
    ...
    metadata_dict = {'Theta':['Potential Temperature', '°C', cm.balance, -1, 1],
                     'Salt':['Salinity', 'PSU', cm.balance, -1, 1]}
    ...
    for month in range(6,10):
        ...
        monthly_mean_control = np.nanmean(var_grid_control[month_indices, :, :], axis=0)
        monthly_mean_anomaly = monthly_mean - monthly_mean_control

        # Plot
        plt.pcolormesh(distance_along_transect, Z, monthly_mean_anomaly, shading='auto', cmap = metadata_dict[var_name][2],
                          vmin=metadata_dict[var_name][3], vmax=metadata_dict[var_name][4])
        plt.contour(distance_along_transect, Z, monthly_mean_anomaly,
    ...
    plt.savefig(os.path.join(project_folder, 'Figures','Model',transect, f'{experiment}_{var_name}_anomaly_vs_control_{transect}_{year}.png'))
    plt.close(fig)
...
year = 2024
month = 8
experiment = 'prescribed_seaice'
experiment = 'daily_hydrology'

anomaly = True # anomaly is relative to control
    ...
    for year in [2023, 2024]:
        for var_name in ['Theta','Salt']:
            for transect in ['BS','DBO3','DBO5']:
                ...
                dec_yrs, Z, var_grid = read_modeled_DBO_section(project_folder, var_name, experiment, transect)
                _, _, var_grid_control = read_modeled_DBO_section(project_folder, var_name, 'control', transect)

                coordinates_4326 = get_DBO_coordinates(transect)
                coordinates_32602 = reproject_points(get_DBO_coordinates(transect), 4326, 32602)

                # this anomaly compares inter-experiment differences to control
                plot_monthly_experiment_anomaly_DBO_fields(project_folder, experiment, year, transect, var_name, var_grid_control,
                                             dec_yrs, Z, var_grid,
                                             coordinates_4326, coordinates_32602)
