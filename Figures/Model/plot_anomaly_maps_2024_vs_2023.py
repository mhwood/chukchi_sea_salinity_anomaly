...
def plot_anomaly_panel(project_folder, experiment, var_name, data, month, x, y, img, Depth):
    ...
    file_name = var_name+'_2024_2023_'+month_name+'_anomaly_'+experiment+'.png'
    output_folder = os.path.join(project_folder, 'Figures','Anomaly Maps  2024 vs 2023')
    ...
    if experiment=='control':
        experiment_long_name = 'Climatological Hydrology & Model-Evolving Sea Ice'
    if experiment=='prescribed_seaice':
        experiment_long_name = 'Daily Hydrology & Nudged Sea Ice'

    if 'SI' in var_name:
        plt.title(plot_metadata[var_name]['long_name'] + ' Anomaly (2024 - 2023)',
                  fontsize=12)
    else:
        plt.title(plot_metadata[var_name]['long_name'] + ' Anomaly (2024 - 2023, 5m depth)'+\
                  '\nExperiment: ' + experiment_long_name,
                  fontsize=12)
...
for experiment in ['control','prescribed_seaice','daily_hydrology']:
    ...
    for var_name in ['Theta','Salt']:
        ...
        for month in [6,7,8,9,10]:
            ...
            data_2023 = read_model_field_from_nc(config_dir, results_dir, var_name, 2023, month)
            data_2024 = read_model_field_from_nc(config_dir, results_dir, var_name, 2024, month)

            data_difference = data_2024 - data_2023
            data_difference = np.mean(data_difference, axis=0)

            plot_anomaly_panel(project_folder, experiment, var_name, data_difference, month, x, y, img, Depth)
