...
def read_seaice_anomaly_from_nc(project_dir, month):
    ds = nc4.Dataset(os.path.join(project_dir,'Data', 'Observations', 'Sea_ice_Anomaly_2025_'+month+'.nc'))
    X = ds.variables['Longitude'][:,:]
    Y = ds.variables['Latitude'][:,:]
    seaice_anomaly = ds.variables['seaice_anomaly'][:,:]
    ds.close()
    return X, Y, seaice_anomaly

def plot_panel(project_folder, x, y, img, X, Y, Depth, X_seaice, Y_seaice, seaice_anomaly, month):

    file_name = 'Chukchi_Sea_seaice_Anomaly_'+month+'.png'
    ...
    vmin = -0.5
    vmax = 0.5
    units = 'Sea Ice Concentration Anomaly (fraction)'

    gs = GridSpec(17, 11, figure=fig, left = 0.02, right = 0.99, top = 0.91, bottom = 0.08)

    ax = fig.add_subplot(gs[:, :])

    # plot_grid = np.ma.masked_where(seaice_anomaly)
    ...
    C = plt.imshow(seaice_anomaly, extent=(X_seaice.min(), X_seaice.max(), Y_seaice.min(), Y_seaice.max()), cmap=cmap,
                   origin='lower',
    ...
    plt.title('Sea Ice Anomaly - '+month +' 2024 vs 2015-2024 Average',
              fontsize=12)
...
X_seaice, Y_seaice, seaice_anomaly = read_seaice_anomaly_from_nc(project_dir, month)

plot_panel(project_dir, x, y, img, X, Y, Depth, X_seaice, Y_seaice, seaice_anomaly, month)
